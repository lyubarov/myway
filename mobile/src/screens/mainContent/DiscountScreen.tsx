...
import { getProductsFromBlockById } from "src/firebase/db";
type AuthNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  "MainContent"
>;

const DiscountScreen = ({ blockId }: { blockId: string }) => {
  const navigation = useNavigation<AuthNavigationProp>();
  const { isDarkMode } = useAuth();

  const { handleAddToBasket, cartProducts, products } = useUsersProduct();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const newData = await getProductsFromBlockById(blockId);
        const discountData = products.filter((product) => product.discount);
        const allProducts = [...newData, ...discountData];
        ...
        if (JSON.stringify(uniqueProducts) !== storedProducts) {
            ...
            <View className="flex-row">
              ...
              >
                <LinearGradient
                  ...
                  className="py-3 rounded-full w-[90px]"
                >
                  <Text
                    allowFontScaling={false}
